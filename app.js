...
function buildReceiptPdfBase64({ name, montant, beneficiaire, compte, reference, statusLabel = 'ZREALIZOWANY', statusColor = '#059669' }) {
  ...
  const text = (x, y, size, value, rgb = '0.12 0.16 0.23', bold = false) => {
    content.push(`${rgb} rg BT /${bold ? 'F2' : 'F1'} ${size} Tf ${x} ${y} Td (${pdfEscape(value)}) Tj ET`);
  };
  ...
  for (const [label, value] of rows) {
    text(42, y, 10, label.toUpperCase(), '0.39 0.45 0.55', true);
    text(220, y, 11, value, '0.12 0.16 0.23', label === 'Montant');
    content.push('0.88 0.90 0.93 RG 42 ' + (y - 13) + ' m 553 ' + (y - 13) + ' l S');
    y -= 52;
  }
...
window.withSpinner = function(action, duration = 1200) {
  let start = Date.now();
...
const themes = {
  ...
  'professional-split': { p: '#2563EB', l: '#DBEAFE' }
};
...
function applyTheme(theme) {
  ...
  if (theme === 'professional-split') applyBgColor('professional-split');
}
  ...
  return `
    ...
    <div class="profile-section-pro">
      <div class="profile-section-header-pro">
        ...
        <button class="profile-edit-link" type="button" onclick="toast('Edycja danych będzie dostępna wkrótce')">Modifier</button>
      </div>
...
window.login = async function(options = { silent: false, redirect: false }) {
  ...
  try {
    ...
    user = f;
    user._id = fid;
    applyBgColor(user.bgColor || (user.theme === 'professional-split' ? 'professional-split' : 'gray'));
